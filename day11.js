...
function adjacentSeats(rowN, seatN) {
    ...
    if (rows[rowN - 1]) {
        if (rows[rowN - 1][seatN - 1]) seats.push(rows[rowN - 1][seatN - 1]);
        seats.push(rows[rowN - 1][seatN]);
        if (rows[rowN - 1][seatN + 1]) seats.push(rows[rowN - 1][seatN + 1]);
    }

    if (rows[rowN][seatN - 1]) seats.push(rows[rowN][seatN - 1]);
    if (rows[rowN][seatN + 1]) seats.push(rows[rowN][seatN + 1]);
    ...
    rows.forEach((row, rowN) => {
        ...
        row.forEach((seat, seatN) => {
            ...
            if (seat === 'L' && seats.filter(value => value === '#').length === 0) {
                newRows[rowN][seatN] = '#';
                changes++;
            } else if (seat === '#' && seats.filter(value => value === '#').length >= 4) {
