...
let valid = 0;
data.forEach((line) => {
    const input = line.match(/([0-9]+)-([0-9]+) (\w+): (.*)/);
    const min = parseInt(input[1], 10);
    const max = parseInt(input[2], 10);
    const needle = input[3];
    const value = input[4].split('');

    const isValid = (value[min - 1] === needle || value[max - 1] === needle)
        && !(value[min - 1] === needle && value[max - 1] === needle);

    if (isValid) valid++;
    console.log(isValid, line, min, max, needle, value);
});

console.log('Valid:', valid);
console.log('Invalid:', data.length - valid);