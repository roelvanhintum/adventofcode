const input = `hcl:5d90f0 cid:270 ecl:#66dc9c hgt:62cm byr:1945 pid:63201172 eyr:2026
...
const parsedData = inputData.map((inputLine) => {
...
inputData.forEach((item) => {
  const isValid = item.indexOf('byr') !== -1 // (Birth Year)
    && item.indexOf('iyr') !== -1 // (Issue Year)
    && item.indexOf('eyr') !== -1 // (Expiration Year)
    && item.indexOf('hgt') !== -1 // (Height)
    && item.indexOf('hcl') !== -1 // (Hair Color)
    && item.indexOf('ecl') !== -1 // (Eye Color)
    && item.indexOf('pid') !== -1; // (Passport ID)
    // && item.indexOf('cid') !== -1; // (Country ID)
...
valid = 0;
inputData.forEach((item) => {
  const isValid = item.match(/(^| |\n)byr:(19[2-8][0-9]|199[0-9]|200[0-2])( |\n|$)/g) !== null // (Birth Year)
    && item.match(/(^| |\n)iyr:(201[0-9]|2020)( |\n|$)/g) !== null // (Issue Year)
    && item.match(/(^| |\n)eyr:(202[0-9]|2030)( |\n|$)/g) !== null // (Expiration Year)
...
console.log('Valid strict:', valid);