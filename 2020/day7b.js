...
let input = `dotted blue bags contain 3 wavy bronze bags, 5 clear tomato bags.
...
input = input.replace(/ bags( |,|\.)/g, ' bag$1');
input = input.split(`
`);

const formattedInput = {};
input.forEach(line => {
    const base = line.split(' contain ')[0];
    let contentsRaw = line.split(' contain ')[1];
    contentsRaw = contentsRaw.substring(0, contentsRaw.length - 1);
    let contents = {};
    contentsRaw.split(', ').forEach(content => {
        const matches = content.match(/^([0-9]+) (.*)$/);

        if (matches) contents[matches[2]] = parseInt(matches[1], 10);
    });

    formattedInput[base] = contents;
});

// console.log(formattedInput);

function searchBags(searchInput) {
    let childBags = 0;
    Object.keys(formattedInput[searchInput]).forEach((key) => {
        childBags += formattedInput[searchInput][key] * searchBags(key);
    });

    return 1 + childBags;
}

const bagsNeeded = searchBags('shiny gold bag') - 1;

console.log(bagsNeeded);
console.log('Total entires:', Object.keys(formattedInput).length);
console.log('Bags needed:', bagsNeeded);