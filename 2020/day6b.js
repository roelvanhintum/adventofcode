// question a-z yes or no
// answered yes written down
...
const answeredQuestions = input.map((groupInput) => {
    const answers = {};
    const memberCount = groupInput.split(`
`).length;
    groupInput.split('').forEach(letter => {
        if (typeof answers[letter] === 'undefined') answers[letter] = 0;
        answers[letter]++;
    });

    let finalAnswers = 0;
    for (const letter in answers) {
        if (answers[letter] === memberCount) finalAnswers++;
...
let result = 0;
answeredQuestions.forEach(answerCount => {
    result += answerCount;
});
